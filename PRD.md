...
- **FR-29 — Containerized distribution.** The app ships as a Docker image — Node runtime + built app + a mid-size TeX Live, so the whole pipeline (scrape → score → rewrite → compile → save) runs inside one container — plus a `docker-compose.yml` that brings up the full stack in one command: the app, a local-scoring Ollama sidecar, and a one-shot init service that pre-pulls the scoring model (tag overridable via `OLLAMA_MODEL`). State survives restarts: the SQLite file and the Ollama model cache live in named volumes, approved resume packages land in a `./output` host bind, and the user's private `resume/` assets can be bind-mounted read-only. Secrets (`ANTHROPIC_API_KEY`, optional `RAPID_API_KEY`) are supplied at run time from the untracked `.env` and are never baked into the image. The two Ollama services sit behind a **default-on `local-scoring` compose profile** (`COMPOSE_PROFILES=local-scoring` in the example `.env`), so the quick-start behavior is unchanged while one blanked variable removes the sidecar for users scoring elsewhere (FR-31). A committed **NVIDIA GPU override** (`docker-compose.gpu.yml`) grants the sidecar the host's GPUs for accelerated local scoring on Windows/Linux; AMD-on-Linux (ROCm image) and no-passthrough platforms are documented recipes rather than committed files. Native (non-Docker) runs remain fully supported and unchanged.
...
- **FR-31 — Configurable Ollama endpoint.** The address of the local scoring backend is a single server-side environment variable, `OLLAMA_BASE_URL` (default `http://127.0.0.1:11434`), resolved in exactly one place — so pointing the app at the compose sidecar, the Docker host, or another machine is configuration, never a code change. In the FR-29 stack this composes with the `local-scoring` profile into a supported **host-Ollama mode**: `COMPOSE_PROFILES=` (empty) plus `OLLAMA_BASE_URL=http://host.docker.internal:11434` starts the app alone and points scoring — including the in-app model status and download — at a natively installed Ollama (the GPU path for Macs and AMD-on-Windows, where Docker has no passthrough), with no redundant in-container model pull.
