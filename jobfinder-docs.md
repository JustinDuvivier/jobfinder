...
The scraper has three realistic implementations: static demo data, an HTTP client against LinkedIn's public guest job API, and the Proxycurl paid API. These are not configuration options over the same code — they have genuinely different dependencies, different failure modes, and different setup requirements.
...
The Setup module is a server-rendered form. Initial values are loaded from SQLite on the server before the page is sent to the browser, so there is no loading flash.
...
| Scoring backend & Ollama model tag | Which backend scores jobs (FR-6): the local Ollama model (default, `user_config.scoring_backend = 'ollama'`) or Anthropic (Haiku). The model tag (`user_config.ollama_model`, default `batiai/qwen3.6-27b:iq3`) names the pulled Ollama model the local backend runs — switching back to Anthropic or trying another local model requires no code edits. |
...
### Strategy C — Proxycurl

Calls Proxycurl's job search API with the LinkedIn search URL. More reliable from a detection standpoint but costs per request. Worth switching to if the guest API starts getting rate-limited or IP-blocked, or if volume grows beyond what cautious request rates allow.
...
**Recommendation:** Use the guest API as the primary source. Run it on demand (or on an interval while the app is open). Switch to Proxycurl only if the guest endpoints get IP-blocked or you outgrow safe request rates. Greenhouse is **additive**, not an alternative to the guest API: turn it on to widen coverage with aggregator-sourced Greenhouse postings running in the same run, and leave it off to run LinkedIn alone.
...
A single SQLite file stores all persistent application state. The schema has eight tables.

**user_config** holds the settings from the Setup module: the LaTeX resume source, the Source of Truth text, the saved-search inputs (`keywords` and `locations`, JSON arrays — the cross product the LinkedIn guest strategy runs, FR-2/FR-25), the Proxycurl `search_url`, the active scraper strategy name, the `greenhouse_enabled` flag (INTEGER 0/1, default 0 — the orthogonal Greenhouse-source toggle, FR-5a, that runs the aggregator source alongside the primary strategy), the owner name, the auto-filter cutoff `score_threshold` (INTEGER, default 50; 0 disables — see FR-9a), and the scoring backend (`scoring_backend`, `'ollama'` default / `'anthropic'`) with its local model tag (`ollama_model`, default `batiai/qwen3.6-27b:iq3` — FR-6). The output base directory lives in the server's environment config. One row per user.
